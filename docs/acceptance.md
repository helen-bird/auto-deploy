...
两条端到端路径及上述生命周期检查完成后，建议在本机记录日期、机器、业务仓库、SHA、日志位置和结果。不要把包含个人机器路径、业务绑定或凭证的验收日志提交到公开仓库。
...
- 这次自动化验证未在 MacBook 安装常驻服务或创建定时任务；不等同于实机 launchd 验收。仓库随后已上传 GitHub。

## Mac mini 实机验收反馈（2026-09-14 记录）

维护者明确反馈：“已经在 Mac mini 上实际验收过了，没什么问题。”据此记录为维护者实机验收通过，不再标记为“尚未进行实机验收”。

本次文档整理没有读取目标机器日志，也未获取具体测试日期、macOS 版本、Python 版本、业务配置和逐项验收证据。因此不补造这些信息，不把这条反馈扩展为所有 macOS/项目组合都已通过测试。上表与复验步骤供其他使用者验证自己的环境。
